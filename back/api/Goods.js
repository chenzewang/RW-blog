...
router.get('/insertAllCategory', async (ctx) => {
  fs.readFile(resolve(__dirname, './data/category.json'), 'utf8', (err, data) => {
    data = JSON.parse(data)
    let saveCount = 0
    ...
    data.RECORDS.map((value, index) => {
      ...
      newCategory.save().then(() => {
        saveCount++
        console.log(`成功,${saveCount}/${data.RECORDS.length}`)
      }).catch(error => {
        console.log('失败：' + error)
      })
    })
...
//insertAllCategorySub
router.get('/insertAllCategorySub', async (ctx) => {
  fs.readFile(resolve(__dirname, './data/category_sub.json'), 'utf8', (err, data) => {
    data = JSON.parse(data)
    let saveCount = 0
...
////***获取商品详细信息的接口 getDetailGoodsInfo
