    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 ">
        <div class="moreTag" @click="tagShowFlag = !tagShowFlag">
          <img
            :class="tagShowFlag ? 'moreTagTransform' : ''"
            src="../../assets/image/icon/moreTag.svg"
          />
        ...
        <div class="row-content">
          <article-item
            v-on:thumbclick="thumbSubmit"
            :article-list="rowitem"
          ></article-item>
          <article-page
            :current-page="currentPage"
            :page-all="currentPageAll"
            v-on:pageprocss="pageProcss"
          ></article-page>
        </div>
      </div>
    </div>
...
import { getArticlePage, getAllArticle } from "@/api/article.js";
export default {
  name: "blog",
  components: {
    blogFoot: blogFoot,
  ...
  methods: {
    ...
    async getArticlePage(isInit, data) {
      let page = 1;
      if (!isInit) {
        page = data.current;
      }
      let res = await getArticlePage({ page: page });
      this.rowitem = res.result;
      this.currentPageAll = res.count;
    ...
    pageProcss(data) {
      ...
      this.getArticlePage(false, data);
    }
  }
};
</script>
...
.slide-more-tag-enter,
.slide-more-tag-leave-to {
