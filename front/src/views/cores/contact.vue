      ...
      <div class="content-container-item">
        <header class="contact-header">
          ...
          <div class="name-introduce">
            <h1>我是陈日王</h1>
            <h5>一名99年生的小菜鸡<font style="font-weight:200"> </font>。</h5>
            <br />
            <div style="position:relative;" class="contact-tools">
              <div
                v-show="wechatFlag"
                style="position:absolute;top:-100%;left:0px;right:0px;"
              >
                <img
                  @mouseleave="wechatFlag = false"
                  width="320px"
                  src="../../assets/image/WechatIMG390.jpeg"
                />
              </div>
              <a
              ...
              <a
                href="https://github.com/xdnloveme"
                target="view_window"
                style="color:#222;margin-left: 10px;"
              >
                <i class="fa fa-github fa-2x" aria-hidden="true"></i>
              </a>
              <a
                href="https://weibo.com/xdnloveme"
                target="view_window"
                style="color:#e4162c;margin-left: 10px;"
              >
          ...
          <hr />
          <div
            v-for="(introduce, index) in introduceList"
            :key="index"
            class="contact-introduce-item"
          >
            <h2>{{ introduce.title }}</h2>
            <p
              v-for="(sectionItem, index) in introduce.section"
              :key="index"
      ...
      <div id="leave_message" class="content-container-item">
        ...
        <div class="comment">
          ...
          <div id="comment" class="comment-list-head">
            评论墙
            <font style="font-size: 14px;color:#c7254e;">(共20条评论)</font>
            <hr />
          </div>
          <comment-wall
            :comment-list="commentList"
            :waterfall-data="waterfallData"
            v-on:waterfall="waterfallTest"
          ></comment-wall>
        </div>
      </div>
    </div>
  ...
  methods: {
    ...
    getConfig() {
      ...
      this.commentList = JSON.parse(
        JSON.stringify(config.data.contact["commentList"])
      );
    ...
    waterfallTest: function(data) {
      ...
      setTimeout(() => {
        ...
        data.isLoading = false;
        if (_self.count === 3) {
          // 如果在服务器端没有数据返回了为空(这里假设获取的数据为空)，那么赋值isFinish为真，这里仅仅为测试
          data.isFinish = true;
        }
...
.view-body-contact {
  ...
  font-family: Optima, "Lucida Sans", Calibri, Candara, Arial,
