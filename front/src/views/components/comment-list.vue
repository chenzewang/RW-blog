<template>
  <ul>
    ...
    <li v-for="(comment, $key) in commentList" :key="$key">
        ...
        <div class="comment-detail-body">
          <div class="comment-detail-body-top">
            ...
            <span style="float:right;color:#aaa;"
              >{{ commentList.length - $key }}楼 &nbsp;{{ comment.date }}</span
            >
          </div>
          <div v-if="comment.response" class="comment-detail-response">
            <h5>
              ...
              <font
                style="font-size: 12px;float: right;color:#aaa;font-weight: 400;"
  ...
  methods: {
    commentResponse: function(index) {
      this.$set(
        ...
        "isResponse",
        !this.commentList[index].isResponse
      );
    },
    submitCommentResponse: function(data, id, index) {
      alert(
        `您点击了回复他人的回话，条目编号为${index}，输出的数据为${JSON.stringify(
          data
        )}，即将回复的条目id为${id}`
      );
      console.log(data);
