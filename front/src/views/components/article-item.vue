    ...
    <div v-for="(item, $key) in articleList" :key="$key" class="content-item">
      ...
      <div class="rowitem-right">
        <div class="rowitem-title">
          ...
          <router-link :to="'/detail/' + item.id"
            ><p style="cursor:pointer;">{{ item.summary }}</p></router-link
          >
        </div>
      </div>
      <div class="rowitem-foot">
        <span
          ...
          {{ item.time.split(" ")[0] }}&nbsp;&nbsp;&nbsp;
          <i class="fa fa-user" aria-hidden="true"></i> {{ item.author }}</span
        >
        <span class="article_taglist">
          <a
            :href="'/blog/' + taglistItem"
            v-for="(taglistItem, index) in articleTaglist[$key]"
            :key="index"
            class="tag"
            >{{ taglistItem }}</a
          >
        ...
        <span class="rowitem-foot-span">
          <a
            :style="item.thumbUpStatus ? 'color:#de686d' : ''"
            @click="thumbupClick(item.article, $key)"
            class="thumbup"
            href="javascript:;"
          >
            ...
            <transition mode="out-in" name="hideToggle">
              <font key="tup" v-if="item.thumbUpStatus">{{
                item.thumbUp
              }}</font>
              <font key="tdown" v-else>{{ item.thumbUp }}</font>
            </transition> </a
          >&nbsp;&nbsp;&nbsp;
          <a class="eyes" href="javascript:;"
...
.content-item {
  ...
  -moz-box-shadow: 0 0.1rem 0.2rem rgba(0, 0, 0, 0.05);
  -o-box-shadow: 0 0.1rem 0.2rem rgba(0, 0, 0, 0.05);
}
.rowitem-left {
...
.content-item:hover {
  ...
  -webkit-box-shadow: 10px 20px 80px rgba(0, 0, 0, 0.2);
  -moz-box-shadow: 10px 20px 80px rgba(0, 0, 0, 0.2);
  -ms-box-shadow: 10px 20px 80px rgba(0, 0, 0, 0.2);
}
.comment,
.comment i,
.eyes,
